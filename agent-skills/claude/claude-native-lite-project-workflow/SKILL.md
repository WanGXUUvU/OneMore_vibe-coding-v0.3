...
- In later sessions, read `CLAUDE.md`, `STATUS.md`, and the current task card first.
...
2. Delivery sessions.
In later sessions, prefer reading `CLAUDE.md`, `STATUS.md`, and the current task card before doing any work. Do not reload the skill unless workflow context is missing, the task needs re-scoping, or the repo must be re-initialized.
...
- In new delivery sessions, prefer continuing from `CLAUDE.md`, `STATUS.md`, and the current task card instead of re-running bootstrap.
