...
- `STATUS.md` for canonical current project state, gate, what is allowed now, current milestone, and next action. `Lane` is optional helper context only. Use [references/status-template.md](references/status-template.md) as the initial format.
...
- After any gate is passed, immediately update `STATUS.md`: set Phase, Task (file path), Gate, Allowed Now, Milestone, and Next action. Set `Lane` only when it adds useful context. Do not declare a gate passed or implementation done before `STATUS.md` reflects the new state.
```
...
- `STATUS.md` with the current phase, gate, allowed action, current milestone, optional lane, and notes
